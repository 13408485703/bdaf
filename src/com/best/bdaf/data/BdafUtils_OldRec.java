...
public class BdafUtils_OldRec {
	...
	public static LocalFlightPlanDB getLocalFlightPlanDBFromBytes(byte[] src, Date recordtime)
	{
		...
		//flight_origin
		n = new Integer(src[6549]&0xe0 >> 5).byteValue();
		lfp.setFlight_origin(E_flight_origin.values()[n].toString());
		
		//creation_time
		System.arraycopy(src, 6537, byte12, 0, 12);
		n = BestUtils.byteToInt(byte12, false);
		lfp.setCreation_time(new Date(1000*((long)n)));
		
		//atd
		System.arraycopy(src, 6567, byte4, 0, 4);
		n = BestUtils.byteToInt(byte4, false);
		lfp.setAtd(new Date(1000*((long)n)));
		
		//ata
		System.arraycopy(src, 6575, byte4, 0, 4);
		n = BestUtils.byteToInt(byte4, false);
		lfp.setAta(new Date(1000*((long)n)));
		
		//etd
		System.arraycopy(src, 6557, byte4, 0, 4);
		n = BestUtils.byteToInt(byte4, false);
		lfp.setEtd(new Date(1000*((long)n)));
		
		//eta
		System.arraycopy(src, 6571, byte4, 0, 4);
		n = BestUtils.byteToInt(byte4, false);
		lfp.setEta(new Date(1000*((long)n)));
		
		//route_field
		String route_field = new String(src, 6662, 156).trim();
		lfp.setRoute_field(route_field);
		
		//assr_code
		System.arraycopy(src, 6554, byte2, 0, 2);
		...
		//pssr_code
		System.arraycopy(src, 6946, byte2, 0, 2);
		...
		//exit_protocol
		n = new Integer(src[6629]&0x07).byteValue();
		lfp.setExit_protocol(E_exit_protocol.values()[n].toString());
		
		//wtc
		n = new Integer(src[6556]&0x07).byteValue();
		lfp.setWtc(E_wake_turbulence.values()[n].toString());
		
		//field10a
		lfp.setField10a(new String(src, 6874, 50).trim());
		
		//field10b
		lfp.setField10b(new String(src, 6924, 20).trim());
		
		//fpl_status  应该是1e的 1..4
		n = new Integer(src[6968]&0x1e >> 1).byteValue();
		if(n >= E_fpl_status.values().length)
			n = 0;
		lfp.setFpl_status(E_fpl_status.values()[n].toString());
		
		//eobt
		System.arraycopy(src, 6832, byte4, 0, 4);
		n = BestUtils.byteToInt(byte4, false);
		...
		lfp.setAircraft_operator_name(new String(src, 7079, 50).trim());
		...
		//aircraft_address
		System.arraycopy(src, 7150, byte4, 0, 4);
                ...
                for (File file2 : files) 
                {
                    ...
                    else 
                    {
                    	if(file2.getName().endsWith("nrpf") && file2.getName().startsWith("REC_FDP_"))
                    	{
                    		...
                    		BdafUtils_OldRec.getComFplDBsFromFile(file2, isDirect2Excel);
			...
			ArrayList<String> choosedsectornames) throws Exception
	{
		...
		//临时
		//将机型用文件去匹配尾流
		Iterator<Entry<String, Integer>> it_actype = actypes.entrySet().iterator();
		while(it_actype.hasNext())
		{
			Entry<String, Integer> entry = it_actype.next();
			String comboactype = entry.getKey();
			int actypeIndex = comboactype.indexOf(lfp.getAircraft_type());
			if(actypeIndex != -1)
			{
				String newWake = comboactype.substring(
					actypeIndex+lfp.getAircraft_type().length()+1);
				lfp.setWtc(newWake);	
				break;
			}
		}
...
//		skp.setField10b(lfp.getField10b());
	...
	public static ArrayList<Skp> getSkpsFromFpls(String exenum, Date time, int durationsecs, ArrayList<String> choosedsectornames, int pilot, ArrayList<LocalFlightPlanDB> lfps) throws Exception
	{
		...
		for(int i=0;i<lfps.size();i++)	
		{
			...
			Skp skp = BdafUtils_OldRec.getSkpFromFpl(lfp, time, durationsecs, choosedsectornames);
			if(skp == null) //不符合筛选条件
				continue;
